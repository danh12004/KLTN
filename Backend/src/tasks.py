...
def scheduled_monitoring_task(app, user_id):
    ...
    with app.app_context():
        ...
        if farm:
            # iot_data = app.iot_service.generate_fake_data(farm.id)
            iot_data = app.iot_service.get_latest_data(farm.id)
            
            monitoring_agent.check_risk_for_farmer(user, iot_data=iot_data)
