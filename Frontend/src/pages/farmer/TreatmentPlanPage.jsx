...
import {
    ...
    RotateCcw, CheckCircle, XCircle, ShieldCheck, Wind, Bug
} from 'lucide-react';
...
const FarmMap = ({ lat, lon, farmArea }) => {
    ...
    const redDotIcon = L.divIcon({
        className: 'custom-red-dot-icon',
        html: '<div style="background-color: red; width: 10px; height: 10px; border-radius: 50%; border: 2px solid white; box-shadow: 0 0 5px rgba(0,0,0,0.5);"></div>', 
        iconSize: [14, 14],
        iconAnchor: [7, 7],
    });
...
const Spinner = ({ size = 'md' }) => {
    ...
    return (
        <div className={`animate-spin rounded-full border-3 border-t-3 border-slate-200 border-t-emerald-600 ${sizeClasses[size]}`}></div>
    );
};
    ...
    return (
        <div className="flex items-start p-3 bg-white rounded-lg shadow-sm border border-slate-100">
            ...
            <div>
                ...
                <p className="text-sm text-slate-800 font-semibold mt-1 whitespace-pre-wrap">{value}</p>
            </div>
        </div>
    );
};
...
const PlanDisplay = ({ plan, title, titleIcon, bgColor = 'bg-white' }) => {
    ...
    const { analysis, treatment_plan, fertilizer_advice, prognosis, main_message } = plan;

    const drugName = plan.drug_name || treatment_plan?.drug_info?.sản_phẩm_tham_khảo;
    const activeIngredient = plan.active_ingredient || treatment_plan?.drug_info?.hoạt_chất;
    const mixingInstruction = plan.mixing_instruction || treatment_plan?.drug_info?.cách_dùng;
    const applicationRate = plan.total_dosage?.application_rate || treatment_plan?.drug_info?.liều_lượng;
    const farmArea = plan.total_dosage?.farm_area;
    const totalVolume = plan.total_dosage?.total_volume;

    let sprayDate = '';
    let sprayTime = '';
    if (treatment_plan?.optimal_spray_day?.date) {
        try {
            const dateObj = new Date(treatment_plan.optimal_spray_day.date);
            sprayDate = dateObj.toLocaleDateString('vi-VN', {
                year: 'numeric', month: 'numeric', day: 'numeric'
            });
            sprayTime = treatment_plan.optimal_spray_day.session;
        } catch (e) {
            sprayDate = treatment_plan.optimal_spray_day.date;
            sprayTime = treatment_plan.optimal_spray_day.session;
        }
    }
    const sprayReason = treatment_plan?.optimal_spray_day?.reason;

    return (
        <div className={`p-6 md:p-10 rounded-2xl shadow-2xl border ${bgColor} border-red-200`}>
            <h2 className="text-3xl font-extrabold text-slate-800 mb-4 border-b pb-4 flex items-center">
                {titleIcon || <ShieldCheck size={28} className="text-red-500 mr-2" />} {title}
            </h2>
            ...
            {(drugName || activeIngredient || mixingInstruction || applicationRate || totalVolume) && (
                <div className="bg-orange-50 p-6 rounded-xl shadow-inner border border-orange-200 mb-6">
                    <h3 className="text-2xl font-extrabold text-orange-700 mb-4 border-b pb-3">
                        <Leaf size={22} className="mr-2 inline text-orange-500" /> Chi Tiết Thuốc Điều Trị
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        <InfoItem icon={<ShieldCheck size={20} />} label="Tên Thuốc" value={drugName} />
                        <InfoItem icon={<Feather size={20} />} label="Hoạt Chất Chính" value={activeIngredient} />
                        <InfoItem icon={<Info size={20} />} label="Liều Lượng Gốc" value={applicationRate} />
                        <InfoItem icon={<Tractor size={20} />} label="Diện tích áp dụng" value={farmArea} />
                        <InfoItem icon={<Tractor size={20} />} label="Tổng lượng dùng" value={totalVolume} />
                        <div className="md:col-span-2 lg:col-span-3">
                            <InfoItem icon={<RotateCcw size={20} />} label="Hướng dẫn pha/phun" value={mixingInstruction} />
                        </div>
                    </div>
                </div>
            )}
            ...
            {analysis && (
                <>
                    <h3 className="text-xl font-bold text-slate-700 mt-6 mb-3 border-b border-slate-100 pb-2">
                        <Info size={20} className="mr-2 inline text-blue-500" /> Phân Tích & Rủi Ro
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                            <h4 className="font-bold text-blue-700 mb-2 flex items-center">
                                <Bug size={18} className="mr-2" /> Đánh giá rủi ro
                            </h4>
                            <p className="text-sm text-slate-700 whitespace-pre-wrap">{analysis.risk_assessment || 'Chưa có phân tích.'}</p>
                        </div>
                        <div className="p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                            <h4 className="font-bold text-yellow-700 mb-2 flex items-center">
                                <Wind size={18} className="mr-2" /> Phân tích thời tiết
                            </h4>
                            <p className="text-sm text-slate-700 whitespace-pre-wrap">{analysis.weather_summary || 'Chưa có phân tích.'}</p>
                        </div>
                    </div>
                </>
            )}

            {treatment_plan && sprayDate && (
                <div className="bg-slate-50 p-6 rounded-xl shadow-inner border border-slate-200 mb-6">
                    <h3 className="text-2xl font-extrabold text-slate-700 mb-4 border-b pb-3">
                        <Target size={22} className="mr-2 inline text-red-500" /> Thời Điểm Vàng Phun Thuốc
                    </h3>
                    
                    <div className="p-4 bg-white rounded-lg border border-emerald-200 shadow-sm hover:shadow-md transition-shadow">
                        <p className="font-extrabold text-lg text-emerald-800 mb-3 uppercase flex items-center">
                            <Clock size={20} className="mr-2 text-emerald-600" /> Thời Gian Đề Xuất
                        </p>
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 text-sm">
                            <div className="border-r border-slate-100 pr-4">
                                <InfoItem
                                    icon={<Clock size={20} />}
                                    label="Ngày Phun"
                                    value={<span className="text-xl text-red-600 font-extrabold">{sprayDate} (Buổi {sprayTime})</span>}
                                />
                            </div>
                            <div className="lg:pl-4">
                                <p className="font-medium text-slate-500 uppercase tracking-wider mb-2">Lý do chọn thời điểm</p>
                                <p className="text-slate-800 leading-relaxed text-sm">{sprayReason}</p>
                            </div>
                        </div>
                    </div>
                    {treatment_plan.additional_actions && (
                        <div className="mt-4">
                            <InfoItem icon={<Tractor size={20} />} label="Hành Động Bổ Sung" value={treatment_plan.additional_actions.join('\n')} />
                        </div>
                    )}
                    {fertilizer_advice && fertilizer_advice.recommendation && (
                        <div className="mt-4 p-4 bg-blue-100 rounded-lg border border-blue-300">
                            <h4 className="font-bold text-blue-700 mb-2 flex items-center">
                                <Leaf size={18} className="mr-2" /> Tư vấn Bón Phân
                            </h4>
                            <p className="text-sm text-slate-700 whitespace-pre-wrap">{fertilizer_advice.recommendation} ({fertilizer_advice.reason})</p>
                        </div>
                    )}
                </div>
            )}
            ...
            {prognosis && (
                <div className="mt-6 p-4 bg-indigo-50 rounded-xl border border-indigo-200 text-indigo-800 shadow-md">
                    <p className="font-bold text-lg flex items-center mb-2">
                        <ChevronRight size={20} className="mr-2" /> Dự Báo Kết Quả:
                    </p>
                    <p className="text-sm font-medium">{prognosis}</p>
                </div>
            )}
        </div>
    );
...
const TreatmentPlanPage = () => {
    ...
    const loadPlanData = useCallback((sessionData, messagePrefix = "") => {
        if (!sessionData || !sessionData.conversation_id) {
            ...
            setStatus('Chưa tải');
            setExecuteMessage('Chưa có kế hoạch điều trị nào. Hãy nhấn "Phân Tích & Tạo Kế Hoạch".');
        ...
        if (sessionData.final_plan_json) {
            try {
                finalPlan = JSON.parse(sessionData.final_plan_json);
            } catch (e) { console.error("Bad final_plan_json", e, sessionData.final_plan_json); }
        }
        if (sessionData.suggested_plan_json) {
             try {
                suggested = JSON.parse(sessionData.suggested_plan_json);
            } catch (e) { console.error("Bad suggested_plan_json", e, sessionData.suggested_plan_json); }
        }

        if (sessionData.plan) {
            finalPlan = sessionData.plan;
        }
        ...
        const sourcePlan = finalPlan || suggested || sessionData;
        ...
        if (suggested) {
            ...
            if (finalPlan) {
                ...
                msg += 'Chưa có kế hoạch điều trị nào. Hãy nhấn "Phân Tích & Tạo Kế Hoạch".';
            }
        }
    ...
    const loadLatestPlan = useCallback(async (isFromNotification = false) => {
        ...
        try {
            const response = await api.get('/user/notifications/latest?plan_type=treatment');
            const latestSession = response.data;
            
            if (latestSession && (latestSession.plan_type === "Giám sát/Xử lý" || latestSession.plan_type === "treatment")) {
            ...
            if (err.response?.status === 404) {
                ...
                console.error("Lỗi khi tải kế hoạch điều trị mới nhất:", err);
                showAlert('error', 'Không thể tải kế hoạch điều trị gần nhất.');
                setError('Không thể tải kế hoạch điều trị gần nhất.');
            }
        } finally {
    ...
    const generatePlan = async () => {
        ...
        try {
            const response = await api.get('/treatment/treatment-plan');
            ...
            const errorMessage = err.response?.data?.error || err.message || 'Không thể tạo kế hoạch điều trị lúc này. Vui lòng thử lại sau.';
    ...
    const updatePlanFromFeedback = async () => {
        ...
        try {
            const response = await api.post('/treatment/treatment-plan/update', {
    ...
    const handlePlanAction = async (action) => {
        ...
        try {
            const response = await api.post('/treatment/treatment-plan/action', {
                conversation_id: conversationId,
                action: action,
            });
        ...
        <div className="animate-fade-in font-sans relative min-h-screen bg-slate-50 pb-16">
            ...
            <div className="max-w-7xl mx-auto p-3">
                <h1 className="text-4xl font-extrabold text-slate-800 mb-2 flex items-center">
                    <ShieldCheck size={32} className="text-red-500 mr-3" /> Kế Hoạch Điều Trị
                </h1>
                <p className="text-slate-500 mb-8 text-lg">Phân tích sâu bệnh và đề xuất phương án xử lý bằng drone.</p>

                <h2 className="text-3xl font-extrabold text-slate-800 mb-4 border-b pb-4 flex items-center">
                    <Target size={28} className="text-red-500 mr-2" /> Vị Trí Bệnh cần xử lý
                </h2>
                <div className="mb-8">
                ...
                <div className="bg-white p-8 rounded-2xl shadow-lg border border-slate-100 text-center mb-8 hover:shadow-xl transition-shadow">
                    <h2 className="text-2xl font-bold text-slate-700">Phân Tích & Tạo Kế Hoạch Điều Trị</h2>
                    <p className="text-slate-500 mt-2 mb-6 max-w-2xl mx-auto">
                        Tận dụng dữ liệu IoT và hình ảnh để phát hiện sâu bệnh và đề xuất phương án xử lý chính xác.
                    </p>
                    <button
                        onClick={generatePlan}
                        disabled={loading || isExecuting || isUpdating || isActing}
                        className="bg-red-600 text-white font-bold py-3 px-10 rounded-full hover:bg-red-700 transition-all disabled:bg-slate-400 flex items-center justify-center mx-auto cursor-pointer shadow-xl shadow-red-300/50 transform hover:scale-105"
                    >
                        {loading ? <Spinner size="sm" /> : (
                            <>
                                <Bug size={20} className="mr-2" />
                                Phân Tích & Tạo Kế Hoạch
                            </>
                        )}
                    </button>
                </div>
                                    ...
                                    <div className="md:col-span-2 bg-slate-100 p-6 rounded-xl border border-slate-200 shadow-inner">
                                        ...
                                        <textarea
                                            ...
                                            placeholder={
                                                ...
                                                "Ví dụ: 'Tôi muốn đổi loại thuốc vì thời tiết sắp mưa...'"
                                            }
                                    ...
                                    <div className="md:col-span-1 flex items-stretch">
                                        ...
                                        >
                                            {isPlanInFinalState ? (
                                                ...
                                                <><Zap size={30} /><span className="mt-2">THỰC THI PHUN THUỐC</span><span className="text-xs mt-1 font-medium">(Gửi lệnh đến Drone/Van)</span></>
                                            )}
                                        </button>
                                    </div>
                                </div>
