...
const Spinner = ({ size = 'md', color = 'emerald' }) => {
    ...
    const colorClasses = {
        emerald: 'border-emerald-500', white: 'border-white',
    };
    return (
...
const SettingsPage = () => {
    ...
    const [selectedStore, setSelectedStore] = useState('general_qa'); 
        ...
        const fetchData = async () => {
            try {
                ...
                const formattedFarmInfo = {
                    ...farmData,
                    planting_date: farmData.planting_date ? new Date(farmData.planting_date).toISOString().split('T')[0] : '',
                    rice_variety: String(farmData.rice_variety || ''),
                    soil_type: String(farmData.soil_type || '')
                };
                
                const formattedSettings = {
                    enabled: !!settingsData.enabled,
                    interval: parseInt(settingsData.interval, 10) || 24
                };
    ...
    const handleSave = async (e) => {
        ...
        const dataToSave = {
            farm_info: farmInfo,
            notification_settings: settingsToSend 
        };
        ...
        try {
            ...
            const newOriginalSettings = { ...settings, interval: settingsToSend.interval };
    ...
    const handleKnowledgeUpload = async (e) => {
        e.preventDefault();
        if (!knowledgeFile) {
            setNotification({ message: 'Vui lòng chọn tệp tin để cập nhật.', type: 'error' });
            return;
        }
        ...
        formData.append('store_type', selectedStore);

        try {
            const res = await api.post('/user/knowledge-upload', formData, {
                headers: {
            ...
            console.error("Failed to upload knowledge:", error.response?.data || error);
            const errorMessage = error.response?.data?.error || 'Lỗi không xác định khi cập nhật kiến thức.';
    ...
    return (
        <div className="max-w-2xl mx-auto p-4 sm:p-6 bg-slate-50 min-h-screen">
            ...
            <form onSubmit={handleSave} className="space-y-8">
                ...
                <div className="bg-white p-6 sm:p-8 rounded-xl shadow-md border border-slate-200">
                    <h2 className="text-xl font-bold text-slate-700 mb-4">Cập nhật Tri thức & Tài liệu</h2>
                    <p className="text-sm text-slate-600 mb-4">Tải lên file (TXT, JSON) để bổ sung kiến thức thực tế cho Trợ lý AI.</p>
                    
                    <form onSubmit={handleKnowledgeUpload} className="space-y-4 border p-4 rounded-lg bg-slate-50">
                        <SelectField 
                            label="Kho vector cần cập nhật" 
                            name="store_type" 
                            value={selectedStore} 
                            onChange={(e) => setSelectedStore(e.target.value)}
                            required
                        >
                            <option value="general_qa">Kiến thức chung (Hỏi/Đáp)</option>
                            <option value="fertilizer_management">Quản lý Bón phân</option>
                            <option value="disease_treatment">Điều trị Bệnh</option>
                        </SelectField>

                        <InputField 
                            label={`Chọn tệp tin: ${knowledgeFile?.name || 'Chưa chọn file'}`}
                            type="file" 
                        ...
                        <div className="flex justify-end pt-2">
                            <button 
                                type="submit" 
                                disabled={uploading || !knowledgeFile}
                                className="px-6 py-2 rounded-lg bg-indigo-600 text-white font-semibold hover:bg-indigo-700 disabled:bg-slate-400 flex items-center justify-center gap-2 transition-colors"
                            >
                                {uploading ? <Spinner size="sm" color="white" /> : <UploadCloud className="w-5 h-5"/>}
                                {uploading ? 'Đang xử lý...' : 'Tải lên và Cập nhật'}
                            </button>
                        </div>
                    </form>
                </div>
